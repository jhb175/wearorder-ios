...
| 7 | 预设穿搭 | OOTD 保留为预设库，一套预设可重复安排到未来日期 | 约 95% | 真机检查一键安排日期、同日已有计划提示和计划列表展示 |
| 8 | 计划上下文 | 日常、特殊日子、旅行分别记录地点和天气城市 | 约 90% | 后续接入 MapKit 地点选择和未来天气预取 |
| 9 | AI/天气接口预留 | 计划拥有目的地、日期、天气城市等 AI 所需上下文 | 约 90% | AI Pro 开发时从计划上下文读取，不再改主模型 |
...
- 第 9 阶段继续补齐：从 OOTD 预设卡片或详情页可直接“安排到日期”，自动推断日常/特别日/旅行类型，同日已有计划时提示新增不覆盖。
- 当前打包建议版本：`1.0.0 (13)`。下一次重新上传 TestFlight 需要继续递增 Build Number。
- 仍然建议 TestFlight 真机重点看：添加衣物卡顿、批量导入、天气权限/城市天气、CloudKit 同步、OOTD 预设安排到未来日期。
...
- 已支持从 OOTD 预设卡片或详情页直接安排到日期，保留原 OOTD，不覆盖同日已有计划。
